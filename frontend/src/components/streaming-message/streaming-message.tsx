...
export function StreamingMessage({ userMessage, citations, partialText, onStop }: StreamingMessageProps) {
  // Phase driven by what has arrived from the server, not a mock timer.
  const phase: 0 | 1 | 2 = partialText.length > 0 ? 2 : citations.length > 0 ? 1 : 0;

  const phaseLabel =
    phase === 0 ? "Retrieving…" :
    phase === 1 ? `Reading ${citations.length} source${citations.length === 1 ? "" : "s"}…` :
    "Generating answer…";
